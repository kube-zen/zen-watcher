...
IMAGE_TAG ?= $(VERSION)
# Path to helm-charts repository (must be set explicitly or cloned)
# Clone with: git clone https://github.com/kube-zen/helm-charts.git
# Then set: export CHARTS_REPO=/path/to/helm-charts
CHARTS_REPO ?=
...
sync-crd-to-chart:
	...
	@if [ -z "$(CHARTS_REPO)" ]; then \
		echo "$(RED)❌ CHARTS_REPO environment variable not set$(NC)"; \
		echo "   Clone helm-charts: git clone https://github.com/kube-zen/helm-charts.git"; \
	...
	@if [ ! -d "$(CHARTS_REPO)" ]; then \
		...
		echo "   Clone with: git clone https://github.com/kube-zen/helm-charts.git"; \
		exit 1; \
	fi
