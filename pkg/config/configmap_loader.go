...
func (cml *ConfigMapLoader) Start(ctx context.Context) error {
	...
	logger.WithContext(ctx).Info("Starting ConfigMap watcher for filter config",
	...
	if err != nil {
		logger.WithContext(ctx).Warn("Failed to load initial filter config, will retry on ConfigMap creation",
		...
		logger.WithContext(ctx).Info("Loaded initial filter configuration from ConfigMap",
			sdklog.Operation("configmap_load_initial"),
			sdklog.String("namespace", cml.configMapNamespace),
			sdklog.String("configmap_name", cml.configMapName))
	}
	...
	logger.WithContext(ctx).Info("ConfigMap watcher started and synced",
