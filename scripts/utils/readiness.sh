...
wait_for_grafana_api() {
    ...
    while [ $(($(date +%s) - start_time)) -lt "$timeout" ]; do
        if kubectl exec -n "$namespace" deployment/grafana -- curl -s localhost:3000/api/health >/dev/null 2>&1; then
...
check_ingress_readiness() {
    ...
    if kubectl wait --for=condition=ready pod -n "$namespace" -l app.kubernetes.io/name=ingress-nginx --timeout="${timeout}s" 2>/dev/null; then
...
wait_for_zen_watcher() {
    ...
    if ! kubectl wait --for=condition=ready pod -n "$namespace" -l app=zen-watcher --timeout="${timeout}s" 2>/dev/null; then
    ...
    local start_time=$(date +%s)
    while [ $(($(date +%s) - start_time)) -lt "$timeout" ]; do
        local pod_name=$(kubectl get pod -n "$namespace" -l app=zen-watcher -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
        if [ -n "$pod_name" ]; then
            local phase=$(kubectl get pod -n "$namespace" "$pod_name" -o jsonpath='{.status.phase}' 2>/dev/null || echo "")
            if [ "$phase" = "Running" ]; then
                # Check if metrics endpoint is responding
                if kubectl exec -n "$namespace" "$pod_name" -- curl -s localhost:9090/metrics >/dev/null 2>&1; then
...
wait_for_crd() {
    ...
    if kubectl wait --for=condition=established crd/"$crd_name" --timeout="${timeout}s" 2>/dev/null; then
