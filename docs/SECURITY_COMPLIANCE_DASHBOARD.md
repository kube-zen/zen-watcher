...
**Date**: 2025-12-08  
**Target Users**: Compliance Officers, Security Analysts, Risk Managers
...
## 12. Integration with Zen Watcher

### 12.1 Observation CRD Integration

The Security Compliance Dashboard will leverage Zen Watcher's existing `Observation` CRD infrastructure:

- **Policy Violations** → `Observation` CRDs with `category: compliance`
- **Security Events** → `Observation` CRDs with `category: security`
- **Audit Events** → `Observation` CRDs with `category: audit`
- **Threat Detection** → `Observation` CRDs with `category: threat`

### 12.2 Source Adapters

New source adapters for compliance data:
- **Compliance Scanner Adapter**: Scans for policy violations
- **Audit Log Adapter**: Ingests audit trail data
- **SIEM Adapter**: Integrates with SIEM platforms
- **Compliance Framework Adapter**: Tracks framework-specific requirements

### 12.3 Metrics Integration

Extend existing Prometheus metrics:
- `zen_watcher_compliance_violations_total{framework, severity, category}`
- `zen_watcher_audit_events_total{event_type, source, user}`
- `zen_watcher_threat_detections_total{threat_type, severity, source}`
...
- [Zen Watcher Architecture](./ARCHITECTURE.md)
- [Source Adapters Guide](./SOURCE_ADAPTERS.md)
- [Dashboard Guide](../config/dashboards/README.md)
- [Observations CRD](./CRD.md)
