...
echo "→ Creating demo Observation CRDs..."
...
echo "→ Deploying demo manifests for Checkov scanning..."
...
echo "→ Deploying mock metrics server..."
kubectl apply -f - <<EOF
apiVersion: v1
kind: Pod
metadata:
...
echo "✓ Mock metrics server deployed"
echo "→ Waiting for pod..."
kubectl wait --for=condition=ready pod/zen-watcher-mock -n ${NAMESPACE} --timeout=60s 2>/dev/null || echo "⚠ Pod may take longer to start"
